...
class Empresa extends Agendabase {
	...
	function guardar(){
		...
		if ($this->input->is_ajax_request()){
				...
				if($_POST["codagenda"]==""){
		            ...
		            if ($insertar == 1) {
						...
						if($enviar_cliente == 1){
		                	...
		                	$respuesta_cliente = $enviar_cliente;
		                }
			...
			$data = array(); 
			$data["estado"] = (int)$estado; $data["respuesta_cliente"] = $respuesta_cliente; $data["respuesta_empleado"] = $respuesta_empleado;
			echo json_encode($data);
		}else{
			header("Location: ".base_url());
		}

...
function lista(){
    ...
    $("#acciones").show();
    $.get(url+"clientes/lista", function(data) {
...
function nuevo(){
    ...
    $("#acciones").hide();
    $.get(url+"clientes/nuevo", function(data) {
        $("#contenido").empty().html(data);
    });
}
        ...
        if (result.isConfirmed) {
            $.ajax({
                ...
                success: function(data){
                    console.log(data);
                    if (data==1) {
                        alerta("success", "Guardado", "El Cliente ha sido creado Correctamente.");
                    }else if (data==2) {
                        alerta("success", "Guardado", "El Cliente ha sido modificado Correctamente.");
                    } else{
...
function modificar(){
    if (idform==0) {
        ...
        $("#acciones").hide();
        $.get(url+"clientes/nuevo",function(data){
            $.get(url+"clientes/modificar/"+idform,function(data1){
                $("#contenido").empty().html(data);
...
function eliminar(){
    if (idform==0) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.get(url+"clientes/eliminar/"+idform, function(data){
                    if (data==1) {
                        alerta("success", "Eliminado", "El Cliente  ha sido Eliminado.");
                    }else{
                        alerta("error", "Oops...", "Algo salió mal!");
                    }
                    lista();
...
function ver() {
    if (idform==0) {
        ...
        // $("#titulo").text(" PERFIL CLIENTE ");
        // $("#acciones").hide();
        // $.get(url+"clientes/perfil/"+idform, function(data) {
        //     $("#contenido").empty().html(data);
        // });
    }    
}